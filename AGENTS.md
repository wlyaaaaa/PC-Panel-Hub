...
- 不为测试随意重置 USB root hub、整棵 USB 树或无关设备。HS2 Code 43 恢复仅允许使用健康状态已记录且唯一匹配的专用 hub、端口 2 子节点及 LIAN LI sibling 拓扑；缺失或歧义时失败关闭。
- 只有 L-Connect 回读结果明确为 `Verified=true` 才能把 HS2 记为 Active，并据此启动叠加层或窗口保护。
